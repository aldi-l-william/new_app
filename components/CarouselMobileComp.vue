...
<template>
    <carousel ref="carouselRef" v-bind="config" v-if="isShowCarousel" v-model="currentSlide"> 
      <slide v-for="slide in hotelStore.propertyDetailHotel?.offers[props.index]?.room_images" :key="slide">
        <img :src="slide.size_sm" class="w-full max-h-[200px] h-[250px]"/>
      </slide>
    </carousel>
    ...
    <div class="absolute bottom-1 -right-4 transform -translate-x-1/2 font-semibold bg-gray-900 px-2 py-1 rounded-xl">
